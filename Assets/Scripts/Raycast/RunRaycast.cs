...
var viewSpaceCoordinate = new Vector2(0.5f, 0.35f);
        // Perform a ray cast
        var raycastResult = _environmentDepthAccess.RaycastViewSpaceBlocking(viewSpaceCoordinate);

        Vector3 actualPos;
        actualPos = new Vector3 (raycastResult.Position.x/3 + 2*_leftControllerAnchor.transform.position.x/3, raycastResult.Position.y/3 + 2*_leftControllerAnchor.transform.position.y/3, raycastResult.Position.z/3 + 2*_leftControllerAnchor.transform.position.z/3);
